...
for(month_predict in 12:1){
  ...
  lasso <- cv.glmnet(train_x, train_y, type.measure = 'mae', parallel = T, nfolds = 3)
  preds_col <- paste0("preds_", month_predict)
  
  # Evaluate
  
  target_tbl[[preds_col]] <- as.vector(predict(lasso, test_x))
  
  train_mae <- mean(abs(predict(lasso, train_x) - train_y))
...
target_tbl_glm <- target_tbl
save(target_tbl_glm, file = 'data/target_tbl_glm.RData')
