...
for(month_predict in 12:1){
  ...
  target_tbl[[preds_col]] <- predict(rf_fit, test_x)$.pred
  
  train_mae <- mean(abs(predict(rf_fit, train_x)$.pred - train_y))
...
target_tbl_rf <- target_tbl
save(target_tbl_rf, file = 'data/target_tbl_rf.RData')
