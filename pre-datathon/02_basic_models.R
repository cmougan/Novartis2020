...
add_rolling_stats <- function(sales_tbl, n_before, n_after) {
  ...
  sales_tbl <- sales_tbl %>% 
    ...
    mutate(
      ...
      !! var_name_mean := slider::slide_dbl(target, mean, .before = n_before, .after = -n_after),
      !! var_name_median := slider::slide_dbl(target, median, .before = n_before, .after = -n_after)
    )
...
# Main statistics comparing -------------------------------------------------------------------

sales_tbl %>% 
  ggplot(aes(x = cbc_mean, y = target)) + 
  geom_point()

sales_tbl %>% 
  ggplot(aes(x = month_mean_before_Inf_after_1, y = target)) + 
  geom_point()

# Last month is a very good variable, except when 0
sales_tbl %>% 
  ggplot(aes(x = month_last_before_Inf_after_1, y = target)) + 
  geom_point()


sales_tbl %>% 
  ggplot(aes(x = month_median_before_Inf_after_1, y = target)) + 
  geom_point()
...
mean(abs(predict(xgb_fit, test_x)$.pred - test_y))
# In train it is 86
# Without bag impute, in train Xgboost mean error is 89
# Xgboost mean abs error is 106
# Without bag impute Xgboost mean abs error is 109

mean(abs(predict(rf_fit, train_x)$.pred - train_y))
mean(abs(predict(rf_fit, test_x)$.pred - test_y))
# RF: 65 vs 99
...
mean(abs(predict(glm_fit, test_x, s = 'lambda.min') - test_y))

mean(abs(predict(glm_fit, train_x, s = 'lambda.1se') - train_y))
mean(abs(predict(glm_fit, test_x, s = 'lambda.1se') - test_y))
