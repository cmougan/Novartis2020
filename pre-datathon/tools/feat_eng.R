...
add_rolling_num_days_since <- function(sales_tbl, n_before, n_after) {
  ...
  sales_tbl <- sales_tbl %>% 
    ...
    mutate(
      ...
      !! var_name_0 := slider::slide_dbl(target, ~ min_which(. > 0), .before = n_before, .after = -n_after),
      !! var_name_40 := slider::slide_dbl(target, ~ min_which(. > 40), .before = n_before, .after = -n_after),
      !! var_name_200 := slider::slide_dbl(target, ~ min_which(. > 200), .before = n_before, .after = -n_after)
    )
